...
public class Simulator implements WorldListener {
	...
	private ArrayList<Citizen> alreadyDied;
	private ArrayList<ResidentialBuilding> alreadyCollapsed;
	...
	public Simulator(SOSListener l) throws Exception {
		...
		alreadyDied=new ArrayList<Citizen>();
		alreadyCollapsed=new ArrayList<ResidentialBuilding>();
	...
	public static void main (String[] args) throws Exception {
		...
		System.out.print(sim.emergencyUnits.get(2));
	}
	//*****NOT SURE******************************************
	...
	public ArrayList<Citizen> getAlreadyDied() {
		return alreadyDied;
	}

	public ArrayList<ResidentialBuilding> getAlreadyCollapsed() {
		return alreadyCollapsed;
	}

	public String printDisasters() {
		String s="Past Disasters:"+"\n";
	...
	public String printActiveDisasters() {
	...
	public String printLog() {
		String s="At Cycle Step :"+ currentCycle+"\n";
		for(Disaster d:executedDisasters) {
			if(d.getStartCycle()==currentCycle) {
				s+="There was "+d.toString()+"\n";}
			if(d.getTarget() instanceof Citizen&&((Citizen)d.getTarget()).getState()==CitizenState.DECEASED) {
				if(!alreadyDied.contains((Citizen)d.getTarget())) {
				Citizen c=(Citizen)d.getTarget();
				s+="Citizen "+c.getName()+"is DECEASED"+"\n";
				alreadyDied.add(c);
				}
			}
			if(d.getTarget() instanceof ResidentialBuilding&&((ResidentialBuilding)d.getTarget()).getStructuralIntegrity()==0) {
				if(!alreadyCollapsed.contains((ResidentialBuilding)d.getTarget())) {
					ResidentialBuilding c=(ResidentialBuilding)d.getTarget();
					s+="Building at Location (" +c.getLocation().getX()+","+c.getLocation().getY()+")"+"is Collapsed"+"\n";
					alreadyCollapsed.add(c);
					if(c.getOccupants().size()>0)
						s+="Deceased Occupants:" +"\n";
					for(Citizen occ:c.getOccupants()) {
						s+=""+occ.getName()+"\n";
					}
				}
			}
